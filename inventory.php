...
}if($i['itmname'] == "Sunlotion")
{
print " [<a href='itemuse.php?ID={$i['inv_id']}'>Apply</a>]";
}// -- Donation Module -- //
require_once('gateway/donation_functions.php');
$dset = getSettings();

if($i['itmtype'] == $dset['item_type']) {
	echo ' [<a href="donate.php?use_item='.$i['inv_id'].'">Use</a>]';
}
// -- Donation Module -- //
if($i['weapon'])
{
...
if($i['armor'])
{
print " [<a href='equip_armor.php?ID={$i['inv_id']}'>Equip as Armour</a>]";
}
print "</td></tr> ";
}
...
Items with a small green </small><font color='green'>*</font><small> next to their name can be used as armour in combat.</small>
