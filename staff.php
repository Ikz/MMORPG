...
Game Description:<br />
<textarea rows='30' cols='125' name='game_description'>{$set['game_description']}</textarea><br />
...
Will Potion Item: ".item_dropdown($c, "willp_item", $set['willp_item'])."<br />Latest News:<br />
<textarea rows='30' cols='125' name='news'>{$set['news']}</textarea><br />
...
<tr>
<th>DD MMORPG Script Version: </th>
<td>2.6</td>
</tr>
...
{
print "<tr><td>{$r['username']} [{$r['user']}]</td> <td>{$r['action']}</td> <td>".date('g:i:s A, l jS F Y',$r['time'])."</td> <td>{$r['ip']}</td></tr>";
}
...
print "<b>Staff Notepad Updated!</b><br><br>";
stafflog_add("Updated the Staff Notepad");
}
print "<form action='staff.php' method='post'>


<textarea name='pad' rows='30' cols='125'>

".htmlspecialchars($set['staff_pad'])."</textarea><br />
