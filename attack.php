...
{print "This person is in a Safe House in a secret location. You can't hunt them down to kill them, sorry.<br><a href='index.php'>Back</a>";
$h->endpage();exit;}else if($ir['safehouse'] == 1){print "You are in a Safe House, you can't attack others.<br><a href='index.php'>Back</a>";
$h->endpage();exit;}
else if ($ir['hp'] <= 1)
{
...
{
print "<center>You're already in a fight, but some reason can't get back to it...<br />Please wait 1 hour (game time) then you can attack players again.
<br><a href='index.php'>> Go Back <</a>";
$_SESSION['attacklost']=0;
$h->endpage();
exit;
...
if($odata['bguard'] >0)
{
print" <center><b><font color='#FFFF00'>Holy Shit!</b></font> You got your ass kicked by the bodyguard! You were injured and taken to the hospital for $hospit min<br/><br/><a href='index.php'>>Home</a></center>";
$h->endpage();     
$db->query("UPDATE users SET hp=0, hospital=hospital+$hospit, hospreason='Beaten severely by someone's Personal Bodyguard' WHERE userid=$userid",$c);
exit;
}
...
event_add($youdata['userid']," You tried to beat <a href='viewuser.php?u={$odata['userid']}'>{$odata['username']}</a> but stalemated.");
exit;
}
...
{
print "<center>You can only attack those who have atleast 5% health !
<br><a href='index.php'>> Go Back <</a>";
$h->endpage();
exit;
}
...
{
$odata['hp']=0;
$_SESSION['attackwon']=$_GET['ID'];$hitmanq=mysql_query("SELECT * FROM hitman_agency WHERE h_to={$_GET['ID']} AND h_done=0",$c);
if($ir['hitman_user'] > 0 and mysql_num_rows($hitmanq) > 0)
{
$tm=mysql_fetch_array($hitmanq);
mysql_query("UPDATE hitman_agency SET h_done=$userid WHERE h_to={$_GET['ID']} AND h_done=0 LIMIT 1;",$c);
mysql_query("UPDATE users SET money=money+{$tm['h_price']} WHERE userid=$userid",$c);
$text = "You have won \$$tm[h_price]";
if($tm['h_crystals'] > 0){ 
mysql_query("UPDATE users SET crystals=crystals+{$tm['h_crystals']} WHERE userid=$userid",$c);
$text .= " and $tm[h_crystals] crystals";
}
$text .= " For your hitman mission.";
event_add($tm['h_from'],"<a href=viewuser.php?u=$userid>{$ir['username']}</a> completed your hitman mission attack on user ID {$r['h_to']}.");
event_add($userid,$text);
print "<b>Wow, You won a hitman mission and won <b>\$$tm[h_price]</b> and maybe more!!</b><br />";
}
...
$hitmanq=mysql_query("SELECT * FROM hitman_agency WHERE h_to={$_GET['ID']} AND h_done=0");
if($ir['hitman_user'] > 0 and mysql_num_rows($hitmanq) > 0)
{
$db->query("UPDATE hitman_agency SET h_fails=h_fails+1 WHERE h_to={$_GET['ID']} AND h_done=0 LIMIT 1;",$c);
