...
}$wi=$db->query("SELECT ws.*,w.* FROM weather_status ws LEFT JOIN weathers w ON ws.weather=w.w_id LIMIT 1;");
$w=$db->fetch_row($wi);
if($w['remaining_minutes'] == 0)
{
$weather=$db->fetch_row($db->query("SELECT * FROM weathers ORDER BY rand() LIMIT 1;"));
...
if($weather['w_chance'] < $chance)
{
$weather=$db->fetch_row($db->query("SELECT * FROM weathers ORDER BY w_chance DESC LIMIT 1;"));
}
$w2c=$weather['w_id'];
$db->query("UPDATE weather_status SET weather={$w2c} LIMIT 1;");
}
?>
