...
<div class='info_mid'><h2 style='padding-top:10px;'> Editing News</h2></div>
<div><img src='images/info_right.jpg' alt='' /></div> </div>
<div class='generalinfo_simple'><br> <br><br>

<form action='staff_special.php?action=subnews' method='post'>
<textarea rows='30' cols='125' name='newspaper'>$news</textarea><br /><input type='submit' STYLE='color: black;  background-color: white;' value='Change' /></form></div><div><img src='images/generalinfo_btm.jpg' alt='' /></div><br></div></div></div></div></div>";
}
...
The user will receive the benefits of one donator pack.<br />
...
<input type='radio' name='type' value='6' /> Pack 6 (Starter)<br /><input type='submit' STYLE='color: black;  background-color: white;' value='Give User Donator Pack' /></form></div><div><img src='images/generalinfo_btm.jpg' alt='' /></div><br></div></div></div></div></div>";
}
...
{("SELECT * FROM inventory i UPDATE users u LEFT JOIN userstats us ON u.userid=us.userid SET u.money=u.money+35000,u.crystals=u.crystals+160,
us.IQ=us.IQ+180,u.donatordays=u.donatordays+115 WHERE i.inv_userid=u.userid,i.inv_itemid=34,u.userid={$_POST['user']}");$d=115;

item_add(u.userid, 34, 1);}else
 
if($_POST['type']==6)
{
$db->query("UPDATE users u LEFT JOIN userstats us ON u.userid=us.userid SET u.money=u.money+1000,u.crystals=u.crystals+25,u.donatordays=u.donatordays+3 WHERE u.userid={$_POST['user']}");
$d=3;
}

event_add($_POST['user'],"You were given a $d day donator pack (Pack {$_POST['type']}) from the administration.",$c);
$db->query("INSERT INTO willps_accepted VALUES('', {$buyer}, {$for}, '$pack', unix_timestamp(), '$txn_id')", $c);
print "User given a Donator Pack.";
...
print "</table>";
print "<b>Designers</b><br />
<table width=80% class= 'table'><tr style='background:gray'> <th>User</th> <th>Online?</th> <th>Links</th> </tr>";
$q=$db->query("SELECT * FROM users WHERE user_level=4 ORDER BY userid ASC");
while($r=$db->fetch_row($q))
{
if($r['laston'] >= time()-15*60) { $on="<font color=green><b>Online</b></font>"; } else { $on="<font color=red><b>Offline</b></font>"; }
print "\n<tr> <td><a href='viewuser.php?u={$r['userid']}'>{$r['username']}</a> [{$r['userid']}]</td> <td>$on</td> <td><a href='staff_special.php?action=userlevel&amp;level=3&amp;ID={$r['userid']}' >Secretary</a>  &middot; <a href='staff_special.php?action=userlevel&amp;level=5&amp;ID={$r['userid']}' >Assistant</a> &middot; <a href='staff_special.php?action=userlevel&amp;level=1&amp;ID={$r['userid']}' >Member</a></td></tr>";
...
<input type='radio' name='level' value='4' /> Designer<br />
