...
{
echo 'You can't assassinate yourself!';
$h->endpage();
exit;
}
if($ir['money'] <$cost2)
{
echo 'You do not have enough money to assassinate '.$assas['username'].'';
$h->endpage();
exit;
}
...
if($protection['bguard'] >0)
{
echo ' <h2> <font color=red>Assasination FAILED! </font> </h2>  <br><br> <h4>  Your hired assassin is severely beaten by '.$assas['username'].'\'s bodyguard. </h4> ';
$h->endpage();
exit;
}
...
if($assas['hospital'] || $assas['jail'])
{
echo 'This user is in hospital or jail so they can't be assassinated.';
$h->endpage();
exit;
}
...
event_add($assas['userid'],"You were assassinated by <a href='viewuser.php?u={$ir['userid']}' />{$ir['username']}</a>");
